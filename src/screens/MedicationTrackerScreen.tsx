        ...
        {
            {
                ...
                Bell: '🔔',
                ArrowLeft: '⬅️',
                // 💡 MODIFIED: Changed Dropdown icon to a simpler arrow for better visual fit
                Dropdown: '⌄', 
                Edit: '✏️',
                Download: '⬇️',
                CheckCircle: '🟢',
...
const initialMedications: Medication[] = [
    {
        ...
        endDate: 'Dec 24, 24-2025',
        dosesCompleted: 1, // Start with one dose completed
    },
];

// --- NEW COMPONENT: Baby Dropdown Selector ---
interface BabyDropdownSelectorProps {
    babies: Baby[];
    selectedBaby: Baby | null;
    onSelect: (baby: Baby) => void;
}

const BabyDropdownSelector: React.FC<BabyDropdownSelectorProps> = ({ babies, selectedBaby, onSelect }) => {
    const [isDropdownOpen, setIsDropdownOpen] = useState(false);
    const displayText = selectedBaby ? selectedBaby.name : 'Choose a baby';
    ...
    return (
        <View style={trackerStyles.babyDropdownWrapper}>
            <TouchableOpacity
                style={trackerStyles.babyDropdownSimple} // 💡 MODIFIED: Use the new simple style
                onPress={() => setIsDropdownOpen(!isDropdownOpen)}
            >
                {/* 💡 Display placeholder or selected name */}
                <Text
                    style={[
                        trackerStyles.dropdownText,
                        !selectedBaby && trackerStyles.placeholderText // Apply placeholder style
                    ]}
                >
                    {displayText}
                </Text>
                {/* Icon is now visually separate and part of the simple container */}
                <Icon name="Dropdown" size={24} style={trackerStyles.simpleDropdownIcon} />
            </TouchableOpacity>

            {/* Simple Dropdown Menu (Overlay) */}
            {isDropdownOpen && (
                <View style={trackerStyles.dropdownMenu}>
                    {babies.map(baby => (
                        <TouchableOpacity
                            key={baby.id}
                            style={trackerStyles.dropdownItem}
                            onPress={() => handleSelect(baby)}
                        >
                            <Text style={trackerStyles.dropdownItemText}>
                                {baby.name}
                            </Text>
                        </TouchableOpacity>
                    ))}
...
// --- Sub-Components (AdherenceGraph, MedicationSummary, etc. unchanged) ---
                    ...
                    <View style={trackerStyles.scheduleActions}>
                        ...
                        {isCompleted ? (
                            <View style={[trackerStyles.completeButton, trackerStyles.completedStatus]}>
                                <Text style={trackerStyles.completeButtonTextDone}>DONE</Text> {/* Added unique style for completed text */}
                            </View>
                        ) : (
...
const trackerStyles = StyleSheet.create({
    ...
    tabContainerWrapper: {
        ...
        zIndex: 10, // Ensure dropdown sits above content
    },
    // 💡 NEW STYLE: Wrapper to control dropdown position
    babyDropdownWrapper: {
        marginHorizontal: 16,
        // The width of the dropdown toggle itself
        width: 150, 
    },
    // 💡 MODIFIED STYLE: Removed padding, background, and border from the interactive area
    babyDropdownSimple: { 
        flexDirection: 'row',
        justifyContent: 'space-between',
        alignItems: 'center',
        paddingVertical: 4, // Minimal padding to align text
    },
    ...
    simpleDropdownIcon: {
        ...
        fontSize: 20,
        fontWeight: 'bold', // Make the arrow thicker
        marginTop: -5, // Adjust vertical position of the emoji arrow
    },
    placeholderText: {
    ...
    dropdownMenu: {
        position: 'absolute',
        top: 30, // Adjust to sit correctly below the text/icon
        left: 0,
        width: 150,
        ...
        shadowRadius: 5,
        elevation: 6,
        zIndex: 20, // Must be higher than tabContainerWrapper
    },
    dropdownItem: {
        padding: 12,
        borderBottomWidth: 1,
    ...
    completeButtonTextDone: { // Style for the 'DONE' text
        color: '#10B981',
        fontWeight: '600',
        fontSize: 13,
    },
